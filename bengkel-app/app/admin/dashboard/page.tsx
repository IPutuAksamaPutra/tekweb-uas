...
    total: string | number; // Diterima dari CashierController@index
    total_amount?: string | number; // Diterima dari OrderController
    transaction_date?: string; // Dari Transaksi
    updated_at?: string; // Dari Order
    created_at?: string; // Dari Order
}
...
export default function AdminDashboardPage() {
    ...
    async function loadDashboard() {
        ...
        try {
            ...
            // Menjalankan semua fetch secara paralel (Termasuk 2 sumber pendapatan)
            const [productRes, bookingRes, userRes, cashierRes, ordersRes] = await Promise.all([
                ...
                fetch(`${API}/cashier`, { headers: authHeader }), // Transaksi POS
                fetch(`${API}/admin/orders`, { headers: authHeader }), // Orders (Pelunasan)
            ]);
            ...
            // --------------------------------------------------------------------------
            // 🔥 4. Income (Agregasi dari TRANSAKSI POS dan ORDERS) 🔥
            // --------------------------------------------------------------------------
            const cashierData = await cashierRes.json();
            const ordersData = await ordersRes.json();

            const posTransactions: Transaction[] = cashierData?.transactions ?? [];
            // Asumsi Order data ada di properti 'orders' atau 'data'
            const orderPelunasan: Transaction[] = ordersData?.orders ?? ordersData?.data ?? [];
            
            // Gabungkan Transaksi POS dan Order (pelunasan)
            const combinedTransactions: Transaction[] = [
                ...posTransactions,
                // Map Order data ke format yang mirip dengan transaksi, menggunakan kolom total_amount
                ...orderPelunasan.map(o => ({
                    total: o.total_amount || o.total || 0, // Ambil total amount dari order
                    transaction_date: o.updated_at || o.created_at || new Date().toISOString(), // Ambil tanggal terbaru
                }))
            ];
            ...
            combinedTransactions.forEach((t) => {
                // Ambil total amount dari properti 'total' (dari POS) atau properti lain dari order
                const amount = parseFloat(t.total as string || t.total_amount as string || '0'); 
                
                if (isNaN(amount) || amount <= 0) return; // Abaikan transaksi tanpa jumlah valid

                totalIncome += amount;

                // Tentukan tanggal: gunakan transaction_date atau updated_at/created_at
                const dateString = t.transaction_date || t.updated_at || t.created_at; 
            ...
            setError("Gagal memuat data dashboard. Pastikan API server berjalan dan rute `/admin/orders` sudah benar.");
        } finally {
            setLoading(false);
        }
    }
        ...
        <div className="p-8 space-y-8">
            ...
            <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <Link href="/admin/transaksi" className="bg-green-600 hover:bg-green-700 text-white p-6 rounded-xl shadow flex justify-between items-center transition">
