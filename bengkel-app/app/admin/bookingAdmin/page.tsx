...
export default function AdminBookingPage() {
  ...
  const fetchBookings = useCallback(async () => {
    try {
      ...
      // Log untuk debug di console browser (Hapus jika sudah jalan)
      console.log("Data Booking diterima:", data);

      // Sesuaikan dengan struktur JSON Laravel Anda (data.bookings atau data.data atau data saja)
  ...
  async function updateStatus(id: number, newStatus: string) {
    ...
    try {
      ...
      setBookings(previous); // Rollback jika gagal
    }
  }
    ...
    <div className="min-h-screen bg-slate-50 p-6 lg:p-10 text-slate-800">
      <header className="max-w-7xl mx-auto mb-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-2 font-medium">Kelola antrean servis bengkel</p>
        </div>
        <button 
      ...
      <div className="max-w-7xl mx-auto">
        {loading && bookings.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border-2 border-dashed">
            ...
            <p className="font-bold text-slate-400">Menyinkronkan data antrean...</p>
          </div>
        ) : error ? (
          <div className="bg-red-50 border-l-4 border-red-500 p-6 rounded-xl text-red-700">
            <p className="font-bold">Gagal Memuat Data:</p>
            <p className="text-sm">{error}</p>
            <button onClick={fetchBookings} className="mt-3 text-xs underline font-bold">Coba Lagi</button>
          </div>
        ) : bookings.length === 0 ? (
          <div className="bg-white p-20 rounded-4xl text-center border-2 border-dashed">
            ...
            <h3 className="text-xl font-bold text-slate-400">Belum ada booking hari ini.</h3>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {bookings.map((b) => (
              <div key={b.id} className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-md transition-shadow">
                <div className={`h-2 w-full ${b.status?.toLowerCase() === 'confirmed' ? 'bg-green-500' : 'bg-amber-500'}`} />
                <div className="p-6">
                  <div className="flex justify-between items-start mb-4">
                    <div className="p-2 bg-slate-50 rounded-xl">
                      {b.jenis_kendaraan.toLowerCase().includes('mobil') ? <Car size={20} className="text-blue-500"/> : <Bike size={20} className="text-orange-500"/>}
                    </div>
                  ...
                  <h3 className="text-lg font-black text-slate-800 mb-4 truncate">{b.nama_kendaraan}</h3>
                  <div className="space-y-3 text-sm font-medium text-slate-600">
                    <div className="flex items-center gap-3"><User size={16} className="text-slate-400"/> {b.user?.name || `User ID: ${b.user_id}`}</div>
                    <div className="flex items-center gap-3"><Calendar size={16} className="text-slate-400"/> {b.booking_date}</div>
                    <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                        <p className="text-[10px] uppercase font-black text-slate-400 mb-1">Jenis Servis</p>
                        <p className="font-black text-[#234C6A]">{b.jenis_service}</p>
                    </div>
                  </div>
                  <div className="mt-6 pt-4 border-t">
                    <p className="text-[10px] font-black text-slate-400 uppercase mb-2">Aksi Petugas</p>
                    <select
